...
async function obtenerDatosLiga(liga) {
    try {
        let response = await fetch("../JSONs/resultadosboli.json");
...
function mostrarTablaPosiciones(tablaData) {
    ...
    tablaData.forEach((equipo) => {
        ...
        else if (pos <= 6) claseFila = "zona-naranja";
        else if (pos >= 15) claseFila = "zona-naranja";
        else if (pos >= 16) claseFila = "zona-roja";

...
async function obtenerDatosLiga(liga) {
    try {
        let response = await fetch("../JSONs/resultadosing.json");
...
function mostrarTablaPosiciones(tablaData) {
    ...
    tablaData.forEach((equipo) => {
        ...
        else if (pos <= 4) claseFila = "zona-azul";
        else if (pos <= 5) claseFila = "zona-naranja";
        else if (pos >= 18) claseFila = "zona-roja";
    ...
    tabla.innerHTML = `
        ...
        <tbody>
            ${data.map(g => `
                <tr>
                    <td>${g.nombre}</td>
                    <td><img src="${g.escudo}" alt="${g.equipo}" width="30"> ${g.equipo}</td>
                    <td>${g.goles}</td>
                </tr>
            `).join("")}
        </tbody>

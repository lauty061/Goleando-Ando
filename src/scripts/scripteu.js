...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    fechasUnicas = [...new Set(fixtureDataGlobal.map(p => p.jornada || p.fecha_torneo))];
    ...
    loadBracket("../JSONs/resultadoseu.json", "tournament-bracket");
...
function detectCurrentFecha(fixtureData, fechasUnicas) {
    ...
    for (let i = 0; i < fechasUnicas.length; i++) {
        ...
        const partidos = fixtureData.filter(p => (p.fecha_torneo === fechaName) || (p.jornada === fechaName));
        
        if (partidos.length === 0) continue;

        for (let partido of partidos) {
            const fechaStr = partido.fecha || partido.fecha_partido;

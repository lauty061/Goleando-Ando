...
def obtener_fixture_y_tabla(url_fixture, url_tabla, url_goleadores, nombre_liga):
    ...
    bloques = soup.find_all("div", class_="statsWrapPositions")
    for bloque in bloques:
        titulo = bloque.find("h2", class_="stats-top")
        zona_actual = titulo.find("small").text.strip() if titulo and titulo.find("small") else "Zona Desconocida"
        tabla = bloque.find("table", id="posiciones")
        if not tabla:
            continue
        ...
        for fila in filas:
            ...
            equipo_escudo = img_tag.get("data-src") or img_tag.get("src") or "https://upload.wikimedia.org/wikipedia/commons/a/ac/No_image_available.svg"
...
ruta_archivo = os.path.join(carpeta_destino, "resultadosbna.json")

...
def obtener_fixture_y_tabla(url_fixture, url_tabla, url_goleadores, nombre_liga):
    ...
    soup = BeautifulSoup(response.text, 'html.parser')
    tabla_posiciones = []
    grupo_counter = 1  # en caso de que no haya nombre explícito
    grupo_actual = None

    for table in soup.find_all("table", class_="stats-table"):
        zona_th = table.find("th", class_="zona")
        if zona_th:
            grupo_actual = zona_th.get_text(strip=True)  # Usa el nombre que viene, ej: "Grupo A"
        else:
...
ligas = {
    "Mundial de Clubes": {
        "fixture": "https://www.tycsports.com/estadisticas/mundial-de-clubes.html",
        "tabla": "https://www.tycsports.com/estadisticas/mundial-de-clubes.html",
        "goleadores": "https://www.tycsports.com/estadisticas/mundial-de-clubes.html"
    },
}
...
ruta_archivo = os.path.join(carpeta_destino, "resultadosmunclub.json")
